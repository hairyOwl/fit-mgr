/*
 * @Description: 血糖详情页面逻辑
 * @Author: hairyOwl
 * @Date: 2022-03-05 17:07:52
 * @LastEditors: hairyOwl
 * @LastEditTime: 2022-03-18 14:33:15
 */
import { 
    defineComponent,
    onMounted,
...
import { bloodGlucose , inventoryLog} from '@/service';
import { result , formatTimestamp ,formatTimestampPlus} from '@/helpers/utils';
import { message } from 'ant-design-vue';
import { CheckOutlined } from '@ant-design/icons-vue';
import Update from '@/views/BloodGlucose/Update/index.vue'; //编辑模块
    ...
    setup(){
        ...
        */
        ...
        const getDetail = async ()=>{
            const res = await bloodGlucose.detail(id);
            result(res)
                .success(( {data} )=>{
        ...
        const deleteCur = async ()=>{
            const res = await bloodGlucose.deleteOne(id);

            result(res)
                .success(({msg})=>{
                    message.success(msg);
                    router.replace('/blood-glucose'); //回到之前
                });
        };
        const goToPre = async ()=>{
            router.replace('/blood-glucose'); //回到之前
        ...
        //编辑方法
        const updateDetail = async(bloodG)=>{
            Object.assign(detailInfo.value , bloodG);
        };
